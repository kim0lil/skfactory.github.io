...
{% include codepen.html hash="9d7394055497ab5394cf412ed4a74761" title="Enyo Template" %}
...
<project default="copy">
    <target name="build">
        <echo file="dest/build.log" append="true" encoding="utf-8">Building...</echo>
    </target>
    <target name="copy">
    </target>
</project>
...
<project default="copy">
    <target name="build">  
        <echo file="dest/build.log" append="true" encoding="utf-8">Building...</echo>
        <copy file="data.h" tofile="dest/data.h"/>
    </target>
    <target name="copy">
    </target>
</project>
...
{% highlight command or terminal %}
ant build
Buildfile: 빌드 파일 경로

build:
     [copy] Copying 1 file to 빌드 파일 경로/dest
...
아마 dest폴더가 만들어졌으며 해당 폴더 내부에서는 data.h파일과 build.log 파일이 존재할 것입니다.
...
다음으로 `dir`속성을 사용하여 복사할 대상인 `dest` 를 기준 디렉토리로 설정합니다.
...
그러기 위해서는 이제 ant의 막강한 기능인 ant pattern(정규식)을 알아야 할 필요가 있습니다.
